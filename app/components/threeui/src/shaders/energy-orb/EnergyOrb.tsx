...
export function EnergyOrb({ className = "", ...props }: EnergyOrbProps) {
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      // Copy the live WebGL buffer synchronously, before the browser clears it.
      host.dispatchEvent(new CustomEvent("energy-orb-frame", {
        bubbles: true, detail: { canvas, starCanvas, elapsed },
      }));
    ...
    host.addEventListener("energy-orb-redraw", start);
    ...
    return () => {
      stop();
      host.removeEventListener("energy-orb-redraw", start);
